...
class YouTubePodcastServer:
    ...
    def _setup_scheduler(self):
        ...
        try:
            ...
            self.logger.info(f"Scheduled automatic refresh every {interval_hours} hours")
    ...
    def _restart_scheduler(self, new_interval_hours: int):
        ...
        try:
            ...
            self.logger.info(f"Rescheduled automatic refresh to every {new_interval_hours} hours")
            return True
    ...
    def _handle_refresh_status(self):
        ...
        try:
            ...
            if status['running'] and status['start_time']:
                ...
                status['duration'] = int(status['duration']) if status['duration'] else 0
    ...
    def _handle_update_refresh_interval(self):
        ...
        try:
            ...
            if not isinstance(new_interval, (int, float)) or new_interval <= 0:
                return jsonify({"error": "refresh_interval_hours must be a positive number"}), 400
    ...
    def _validate_display_name(self, display_name: str, existing_ids: list = None, existing_display_names: list = None) -> Tuple[bool, str, str]:
        ...
        if not sanitized_id:
            return False, "Display name must contain at least some alphanumeric characters", ""
        
        # Check ID uniqueness
        if existing_ids and sanitized_id in existing_ids:
            return False, f"A channel with similar name already exists (would create same ID: {sanitized_id})", ""
    ...
    def _handle_add_channel(self):
        ...
        try:
            ...
            existing_display_names = [ch.get('display_name', ch['name']) for ch in config.get('channels', [])]
            ...
            url_valid, url_error, channel_info = self._verify_youtube_channel(data['url'])
            if not url_valid:
    ...
    def _handle_update_channel(self, channel_name: str):
        ...
        try:
            ...
            if 'display_name' in data:
                ...
                existing_ids = [ch['name'] for i, ch in enumerate(channels) if i != channel_index]
                existing_display_names = [ch.get('display_name', ch['name']) for i, ch in enumerate(channels) if i != channel_index]
            ...
            if 'display_name' in data:
                ...
                existing_ids = [ch['name'] for i, ch in enumerate(config['channels']) if i != channel_index]
                existing_display_names = [ch.get('display_name', ch['name']) for i, ch in enumerate(config['channels']) if i != channel_index]
    ...
    def _handle_purge_episodes(self, channel_name: str):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Error removing channel directory {channel_video_dir}: {e}")
            ...
            if timestamps_file.exists():
                try:
                    ...
                    if channel_name in timestamps:
                        ...
                        self.logger.info(f"Cleared refresh timestamp for channel: {channel_name}")
                except Exception as e:
    ...
    def _serve_index(self) -> str:
        ...
        for channel in channels:
            ...
            display_name = channel.get('display_name', channel_id.replace('_', ' ').title())
            ...
            episode_count = len(get_video_files(str(channel_dir))) if channel_dir.exists() else 0
            
            
            # Get last refresh time
            last_refresh_raw = refresh_timestamps.get(channel_id)
            last_refresh = self._format_timestamp(last_refresh_raw) if last_refresh_raw else None
            ...
            podcasts.append({
                ...
                'sponsorblock_categories': channel.get('sponsorblock_categories', []),
    ...
    def _serve_video_file(self, channel_name: str, filename: str) -> Response:
        ...
        if range_header:
            return self._serve_partial_content(episode_file, file_size, range_header, mime_type)
        else:
            # Serve full file
            try:
                return send_file(
    ...
    def _serve_partial_content(self, file_path: Path, file_size: int, 
                              range_header: str, mime_type: str = 'video/mp4') -> Response:
    ...
    def run(self, debug: bool = False):
        ...
        job = self.scheduler.get_job('auto_refresh')
        if job and job.next_run_time:
            self.logger.info(f"Automatic refresh scheduled every {interval_hours} hours, next run: {job.next_run_time}")
