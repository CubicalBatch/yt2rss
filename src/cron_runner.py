...
class AutomationRunner:
    ...
    def cleanup_old_videos(self, channel_name: str, max_episodes: int):
        ...
        try:
            ...
            self.logger.info(f"   📊 Found {len(video_files)} episodes in channel directory")
            ...
            if len(video_files) > max_episodes:
                ...
                self.logger.info(f"   🗑️  Need to remove {len(videos_to_remove)} old episodes")
                
                for upload_date, video_id, json_file in videos_to_remove:
                    try:
                        # Remove video file
                        video_file = channel_dir / f"{video_id}.mp4"
                        audio_file = channel_dir / f"{video_id}.m4a"  # Check for audio files too
                        ...
                        self.logger.info(f"     🗑️  Removed {video_id}: {', '.join(removed_files)}")
    ...
    def process_channel(self, channel: Dict[str, Any], global_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if downloaded_count > 0:
                self.logger.info(f"✅ Download phase complete - {downloaded_count} new episodes downloaded")
            else:
    ...
    def run(self) -> int:
        ...
        self.logger.info(f"🕰️  Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        try:
            ...
            for i, channel in enumerate(channels, 1):
                try:
                    ...
                    if self.process_channel(channel, global_config):
                        processed_count += 1
                        self.logger.info(f"[{i}/{len(channels)}] ✅ Channel processed successfully")
                    else:
                        error_count += 1
                        self.logger.error(f"[{i}/{len(channels)}] ❌ Channel processing failed")
                except Exception as e:
                    self.logger.error(f"[{i}/{len(channels)}] ❌ Unexpected error processing channel {channel.get('name', 'unknown')}: {e}")
                    error_count += 1
            ...
            if error_count == 0:
                ...
                self.logger.warning(f"⚠️  Some channels had errors - check logs for details")
